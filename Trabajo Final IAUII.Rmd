...
```{r}
ggplot(arg_data_properati)+
  geom_histogram(aes(x=tiempo))
```

```{r fig.width=16, fig.height=10}
arg_data_properati <- arg_data_properati %>% 
  mutate(anio_mes = str_sub(fecha, 1 , 7)) %>% 
  mutate(x = ymd(glue::glue("{anio_mes}-01")))

arg_data_properati %>% 
  group_by(x) %>%
  summarise(precioprom=round(mean(precio))) %>% 
  ggplot() +
  geom_line(aes(x=x, y=precioprom))+
  geom_point(aes(x=x, y=precioprom))+
  geom_text(aes(x=x, y=precioprom+300000, label=precioprom), size=5)+
  labs(title = "", 
